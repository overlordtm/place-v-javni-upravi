...
def fetch_payout_by_budget_user_group(year: int, month: int) -> pd.DataFrame:
    ...
    df = df.apply(lambda col: col.str.replace('.', '').astype(int), axis=1)
...
def fetch_payout_by_budget_user(year: int, month: int) -> pd.DataFrame:
    COLS = ['group', 'budget_user_id', 'budget_user_name', 'employees', 'gross_salary', 'NA1', 'C', 'NA2', 'D', 'NA3', 'E', 'NA4', 'NA5','NA6', 'I', 'NA7', 'J', 'NA8', 'O', 'NA9']
    ...
    df["budget_user_id"] = df["budget_user_id"].astype(int)
    df["employees"] = df["employees"].astype(int)
    df["gross_salary"] = df["gross_salary"].astype(int)
